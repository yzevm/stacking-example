...
const getDepositInfo = async (account) => {
  try {
    const totalStacked = await contract.methods.totalStacked().call();
    printResult(`totalStacked ${Web3.utils.fromWei(totalStacked.toString(), 'ether')}`);

    printResult('_______');
    printResult('_______');
    printResult('_______');

    printResult(`Deposits info of ${account}`);
    const userDeposits = await contract.methods.getUserAmountOfDeposits(account).call();
    console.log(userDeposits);
    const deps = new Array(Number(userDeposits.toString()));

    console.log(deps);

    for (const [index, value] of deps.entries()) {
      const userDeposit = await contract.methods.getUserDepositInfo(account, index).call();
      printResult(
        `user deposit ${index}: amount ${Web3.utils.fromWei(userDeposit.amount.toString(), 'ether')}, finish ${new Date(
          userDeposit.finish.toString() * 1000
        ).toDateString()}, percent ${userDeposit.percent.toString() / 100}%`
      );
    }

    const deposit = async (planId, amount) => {
      printResult(`deposit(${planId}, ${amount}) sent.`);
      try {
        const validAmount = Web3.utils.toWei(amount, 'ether');
        const result = await contract.methods.deposit(planId, validAmount).send({ from: account });
